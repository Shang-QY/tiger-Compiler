...
%type <var> lvalue
%type <expList> arg args expa exps
%type <ty> ty
%type <dec> dec vardec 
%type <fundec> fundec
...
%type <decList> decs
%type <fieldList> tyfield tyfields
%type <efieldList> recorditem recorditems
...
expa: SEMICOLON exp expa 					{$$=A_ExpList($2, $3);}
	|             							{$$=NULL;}

exps: exp expa 								{$$=A_ExpList($1, $2);}
	|             							{$$=NULL;}

seqexp: exps 								{$$=A_SeqExp(EM_tokPos, $1);}
	  
recorditem: COMMA ID EQ exp recorditem 		{$$=A_EfieldList(A_Efield(S_Symbol($2), $4), $5);}
	|             							{$$=NULL;}

recorditems: ID EQ exp recorditem 			{$$=A_EfieldList(A_Efield(S_Symbol($1), $3), $4);}
	| 			  							{$$=NULL;}
	   
arg: COMMA exp arg 							{$$=A_ExpList($2, $3);}
	|             							{$$=NULL;}

args: exp arg {$$=A_ExpList($1, $2);}
	|             							{$$=NULL;}
